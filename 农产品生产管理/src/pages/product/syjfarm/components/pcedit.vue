    ...
    <div v-if="!isEditOperate">
      <div class="farm-record">
        ...
        <!-- <div class="farm-operate">
          <el-button @click="operate('gd',{})">基 质</el-button>
          <el-button @click="operate('sf',{})">施 肥</el-button>
          <el-button @click="operate('fz',{})">病虫害</el-button>
          <el-button @click="operate('gg',{})">浇 水</el-button>
          <el-button @click="operate('dq',{})">丢 弃</el-button>
        ...
        <div class="f-df " v-if="form.zt==1 && page==='farm'">
          <div class="base-operate f-cp" @click="operate(item.operate,{})" v-for="(item,index) in operations"
               :key="index">
            <div class="f-fs20"><img :src="`./static/syj/p${index+1}.png`" alt="">{{item.title}}</div>
          </div>
        </div>
...
<!--          <div @click="changeTab(1)" :class="[curTab==1?'tab-active':'']">时间轴</div>-->
        ...
        <div v-show="curTab==2" class="table-container">
          <gd-table @operate="operate" ref="gd">栽培基质</gd-table>
          <sf-table @operate="operate" ref="sf">施肥记录</sf-table>
          <fz-table @operate="operate" ref="fz">病虫害防治记录</fz-table>
          <gg-table @operate="operate" ref="gg">浇 水</gg-table>
          <dq-table @operate="operate" ref="dq">丢 弃</dq-table>
...
<script>
  ...
  import dqTable from "./pcTable/dqTable.vue";
  ...
  export default {
    ...
    data() {
      return {
        operations: [
          {title: "基 质", operate: 'gd'},
          {title: "施 肥", operate: 'sf'},
          {title: "病虫害", operate: 'fz'},
          {title: "浇 水", operate: 'gg'},
          {title: "丢 弃", operate: 'dq'},
