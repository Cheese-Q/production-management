...
<template>
    <el-form ref="form" :model="form" label-width="130px" class="form cpwh-edit" :rules="rules" status-icon>
      ...
      <el-input v-model.trim="form.packToProductNum" @input="(value)=>{$nextTick(()=>{form.csl= String(value).slice(0,10)})}" placeholder="请输入" type="number" min="1">
         <template slot="append" v-if="form.dw">{{form.dw}}</template>
      </el-input>
    </el-form-item>

      <!-- 原材料信息 -->
      <div class="sub-title">原材料信息</div>
      ...
      <el-input v-model.trim="form.packNum" placeholder="请输入" type="number" min="0">
        <template slot="append" v-if="form.ncpdw">{{form.ncpdw}}</template>
      </el-input>
    </el-form-item>
