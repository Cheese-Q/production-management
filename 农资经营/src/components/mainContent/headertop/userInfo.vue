    ...
    >
      ...
      >
        <el-form-item label="旧密码" prop="password">
          <el-input type="password" v-model="passwordData.password" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="新密码" prop="newPassword">
          <el-input type="password" v-model="passwordData.newPassword" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="confirmPassword">
          <el-input type="password" v-model="passwordData.confirmPassword" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    updatePwd(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          var params={
            userId:String(this.getUserInfo.bizUserId),
            oldPwd:this.$md5(this.passwordData.password),
            newPwd:this.$md5(this.passwordData.newPassword)
          }
          this.$fetch.api_base.update_pwd(params).then((data)=>{
            if(data.bean=="1"){
