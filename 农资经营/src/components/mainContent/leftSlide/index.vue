...
<template>
  <div id="nav-menu-list" class="f-cp">
    ...
    >
      ...
      <!-- 二级标题 -->
      <div
        class="secondary-menu f-pa f-cp fadeIn"
      ...
      >
        <router-link
          ...
          class="sub-menu"
          active-class="bg-fff"
          :to="i==0?'none':'/'+item.path+'/'+child.path"
          exact
        >
          <i
            :class="[index==6||index==7?'icon-bottom':'icon-top','el-icon-caret-left']"
            v-if="i===0"
          ></i>
    ...
    <div class="f-pa f-tac copy-right" v-once>
      <div>丽耕科技</div>
      <div class="f-fs12 f-mt5">COPYRIGHT &copy; {{curYear}}</div>
    </div>
  </div>
</template>
  ...
  computed: {
    activeMenu(item, index) {
      return function(item, index) {
        ...
        if (!this.currentTab) {
          //当前菜单所属的一级菜单
          return this.$route.matched[0].path == "/" + item.path ||
            (this.$route.path === "/home" && item.path === "/home")
            ? "active"
            : "";
        }
      };
...
.menu-item {
  ...
  .secondary-menu {
    display: none;
    animation: bounceInRight 0.2s ease;
    width: 160px;
    left: 150px;
    background: #f5fafe;
