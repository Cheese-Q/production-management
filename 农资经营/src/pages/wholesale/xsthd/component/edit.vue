<!--
 * @Author: 龚铱白
 * @Date: 2019-08-16 18:29:18
 * @LastEditors: 龚铱白
 * @LastEditTime: 2019-09-03 16:39:37
 * @Description: 
 -->
<template>
  <div>
    <el-form ref="form"
             ...
             :rules="rules"
             status-icon>
      <div class="form-title f-fs18 f-tac f-mb30 f-mt15">{{id?'编辑':'新增'}}批发退货单</div>
      <div class="f-tar"
...
">单 据
      ...
      <el-table :data="form.listDetail"
        ...
        <el-table-column label="商品名称"
                         prop="agriName"></el-table-column>
        <el-table-column label="规格型号"
                         prop="model"></el-table-column>
        <el-table-column label="单价"
                         prop="basezhxsdj"></el-table-column>
        ...
        <el-table-column label="小计"
                         ...
                         prop="jhje">
          <template slot-scope="scope">
            {{isNaN(Number(Number(scope.row.basezhxsdj)*Number(scope.row.thsl)).toFixed(2))?0.00:Number(Number(scope.row.basezhxsdj)*Number(scope.row.thsl)).toFixed(2)}}
          </template>
        </el-table-column>
  ...
  computed: {
    ...
    getTotalPrice: function () {
      ...
      for (var item of array) {
        if (!isNaN(Number(item.basezhxsdj) * Number(item.thsl))) {
          totalPrice += Number(item.basezhxsdj) * Number(item.thsl);
        } else {
          totalPrice += 0;
        }
      }
  ...
  methods: {
      ...
      fetch({
        ...
        .then(res => {
          ...
          for (var i = 0; i < this.form.listDetail.length; i++) {
            this.form.listDetail[i].basezhxsdj = this.form.listDetail[i].dj;
            this.form.listDetail[i].kthsl = this.form.listDetail[i].dqkthsl;
            this.form.listDetail[i].agriName = this.form.listDetail[i].agriProdName;
          }
        })
        .catch(() => {
          this.loading = false;
    ...
    addTrp (rowlist) {
      ...
      fetch({
        ...
        // this.form.jzzt = res.bean.saleStatus;
        this.form.khid = res.bean.cusId;
        this.form.khmc = res.bean.khmc;
        this.form.xgdjid = res.bean.id;
        this.form.xgdjh = res.bean.saleSno;
        let recordsList = res.bean.listDetail;
        for (var i = 0; i < recordsList.length; i++) {
          if (recordsList[i].kthsl != 0) {
            this.form.listDetail.push(recordsList[i]);
            // this.form.listDetail[i].basezhdj = recordsList[i].basezhxsdj;
          } else {
