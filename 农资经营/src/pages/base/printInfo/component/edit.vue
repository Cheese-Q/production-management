      ...
      submitForm() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            this.disabled=true;
            //新增字段
            let { id,sbbid,dyjmc,dyjxh,dyjzk,dyjlx,dyjbh,dyjmy,sim,bz} = this.form;
            //用户参数
            let {bizId,bizName} = this.userInfo;
            //传值参数
            let params = {id,sbbid,dyjmc,dyjxh,dyjzk,dyjlx,dyjbh,dyjmy,sim,bz};
            params.mrdyj=this.form.mrdyj?"1":"2";
            params.qyid=bizId;
            this.$fetch.api_base.dyjx_save(params).then((res)=>{
