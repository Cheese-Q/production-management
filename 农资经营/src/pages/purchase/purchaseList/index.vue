...
<template>
  <div style="position: relative;">
    <div style="position:absolute;top:-5px;right:40px; ">
      <el-button v-if="isEdit || isEdit1 || isEdit2" @click="closeForm(false)" type="primary" size="medium">返回列表
      </el-button>
    </div>
    <div v-if="isEdit == false">
      <v-table v-show="!isEdit && !isEdit1 && !isEdit2" :id="editId" @edit=edit @addCgd=addCgd @submitCgd=submitCgd
               @check="check" ref="table"></v-table>
    </div>
    <div v-else>
      <v-table v-show="isEdit && isEdit1 && isEdit2" :id="editId" @edit=edit @addCgd=addCgd @submitCgd=submitCgd
               @check="check" ref="table"></v-table>
...
<script>
  ...
  import vEdit from './component/editCgd';
