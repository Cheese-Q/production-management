  ...
  <div class="dkfzr">
    ...
    <div class="header f-df f-pr20">
      <div>
        ...
        <el-input placeholder="请输入订单号/供应商名称" @keyup.enter.native=getTableData v-model=findName class="input-with-select f-vab"  size="small">
        </el-input>
        <el-button type="primary" size="small" style="padding:5px 15px;vertical-align: middle;" @click=getTableData><i class="el-icon-search f-fs20"></i></el-button>
      </div>
      <div class="f-f1 f-tar">
        <el-button type="primary" size="medium" @click="$emit('addCgd','')" round style="padding:9px 25px">新 增</el-button>
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.zt == 0">
            <el-button
              size="mini"
              v-if="scope.row.zt == 0"
              @click="addCgd(scope.row)">编辑</el-button>
            <!--<el-button-->
              <!--size="mini"-->
              <!--v-if="scope.row.zt == 0"-->
              <!--@click="submitCgd(scope.row)">审核</el-button>-->
            <el-button
              size="mini"
              type="danger"
              v-if="scope.row.zt == 0"
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      deleteSj(row){
        this.$confirm("此操作将删除该条进货单信息, 是否继续?", "提示", {
      ...
      */
      ...
      getTableData(obj={}){
        ...
        if(this.searchDate == ''||this.searchDate == undefined||this.searchDate == null){
          ...
          this.$fetch.api_purchase.cgrk_list(params).then(res=>{
