...
<template>
  <div class="dkfzr">
    ...
    <div class="header f-df f-pr20">
      <div>
        <label>类型</label>
        <el-select v-model="type" size="small" class="f-vab" placeholder="请选择" @change=getTableData clearable>
          <el-option value="1" label="客户"></el-option>
          <el-option value="2" label="供应商"></el-option>
          <el-option value="3" label="客户和供应商"></el-option>
        </el-select>

        <el-input placeholder="请输入单位名称" @keyup.enter.native=search v-model=findName class="input-with-select f-vab"  size="small">
        </el-input>
        <el-button type="primary" size="small" style="padding:5px 15px" @click=search><i class="el-icon-search f-fs20"></i></el-button>
        <el-checkbox v-model=delFlag class="f-ml20"  @change=getTableData>不显示停用的往来单位</el-checkbox>
      </div>
