<!--
 * @Author: 龚铱白
 * @Date: 2019-08-15 09:16:11
 * @LastEditors: 龚铱白
 * @LastEditTime: 2019-09-03 16:37:20
 * @Description: 
 -->
...
<template>
  <div>
    <el-form ref="form"
             ...
             :rules="rules"
             status-icon>
      <div class="form-title f-fs18 f-tac f-mb30 f-mt15">{{id?'编辑':'新增'}}进货退货单</div>
      <div class="f-tar"
...
">单 据
      ...
      <el-table :data="form.listDetail"
        ...
        <el-table-column label="单价"
                         prop="basezhrkdj"></el-table-column>
        ...
        <el-table-column label="小计"
                         ...
                         prop="jhje">
          <template slot-scope="scope">
            {{isNaN(Number(Number(scope.row.basezhrkdj)*Number(scope.row.thsl)).toFixed(2))?0.00:Number(Number(scope.row.basezhrkdj)*Number(scope.row.thsl)).toFixed(2)}}
          </template>
        </el-table-column>
  ...
  computed: {
    ...
    getTotalPrice: function () {
      ...
      for (var item of array) {
        if (!isNaN(Number(item.basezhrkdj) * Number(item.thsl))) {
          totalPrice += Number(item.basezhrkdj) * Number(item.thsl);
        } else {
          totalPrice += 0;
        }
      }
  ...
  methods: {
      ...
      fetch({
        ...
        .then(res => {
          ...
          for (var i = 0; i < this.form.listDetail.length; i++) {
            this.form.listDetail[i].basezhrkdj = this.form.listDetail[i].dj;
    ...
    submitForm (zt) {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (thNums == '' || thNums == undefined || thNums == null) {
            this.$message.warning("请填写退货数量");
            this.disabled = false;
            return false;
          }
          ...
          // console.log("可退数量" + this.form.kthsl);
          this.disabled = false;
    ...
    addTrp (rowlist) {
      ...
      fetch({
        ...
        this.form.xgdjh = res.bean.jhdh;
        let recordsList = res.bean.records;
        console.log(recordsList);
        for (var i = 0; i < recordsList.length; i++) {
          if (recordsList[i].kthsl > 0) {
            this.form.basezhrkdj = res.bean.records.basezhrkdj;
